    ...
    if (primed) {
        ...
        while (element->getTimestamp() > lastCrossSection + periodUs) {
            ...
            if (element->getInWindow()) {
                ...
                // If we're not in the desired window, then we'll "pretend" that
                // we generated a cross section and went through the process.
                // We'll need to reset the estimate as well.
                for (auto e : estimators) {
                    e->resetEstimate();
                }
            }
