...
exports.find = function (query, options, rowCallback, doneCallback, errorCallback) {
    ...
    var whereClause = common.constructWhereQuery(
        common.mapQueryKeyName(dividedQuery.cassandra, cassandraMap)
    );

    var queryString = 'SELECT * FROM event' + whereClause + ' ALLOW FILTERING';
    ...
    cassandra.execute({
        ...
        rowCallback: function (resource, n) {
            if (common.testAgainstQuery(resource, dividedQuery.local)) {
                rowCallback(resource, ++matches);
            }
        },
