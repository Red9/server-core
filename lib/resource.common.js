...
/**
 * TODO(SRLM): Make sure that strings can't inject code into the database.
 *
 * @param value
 * @returns {string} converted value or undefined if could not convert.
 * @throws {VError} Errors if a non string or number value is passed.
 */
function valueToCassandraString(value) {
    if (_.isDate(value) === true) {
        throw new VError('Cannot pass a date type to this function.');
    } else if (_.isObject(value) === true) {
        throw new VError('Cannot pass an object type to this function.');
    } else if (_.isArray(value) === true) {
        throw new VError('Cannot pass an array type to this function.');
    }

    if (_.isString(value)
        && (/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}$/.test(value)
            // This second regex is a legacy option!!! As it turns out, I haven't been using version 4
        ...
        throw new VError('Unparsable type: %s, value: %s', typeof value, value);
    }
}
...
/**
 *  Should this function check id and table name?
 * @param tableName
 * @param id
 * @param mappedResource
 * @returns {string}
 */
exports.createUpdateString = function (tableName, id, mappedResource) {
    if (_.keys(mappedResource).length === 0) {
        throw new VError('Must have at least one value for creating an update string.');
    }

    var assignments = _.map(mappedResource, function (value, key) {
        return key + ' = ' + valueToCassandraString(value);
    }).join(', ');

    return 'UPDATE ' + tableName + ' SET ' + assignments + ' WHERE id = ' + id;
};
...
/** Checks resource. Does nothing on success. Throws error on failure.
 *
 * @param schema
 * @param resource
 * @throws error on failure with description of what failed
 */
exports.checkNewResourceAgainstSchema = function (schema, resource) {
    // Make sure that the schema is correctly represented in the resource
    _.each(schema, function (parameters, key) {
        if (parameters.includeToCreate) {
            if (!_.has(resource, key)) {
                throw new VError('Must include key "%s" to create.', key);
            } else if (!validateType(parameters.type, resource[key])) {
                throw new VError('Value for key "%s" is not valid.', key);
            }
        } else if ((!parameters.includeToCreate || parameters.dynamic) && _.has(resource, key)) {
            throw new VError('Must not include key "%s" to create.', key);
        }
    });
    ...
    _.each(resource, function (value, key) {
        if (!_.has(schema, key)) {
            throw new VError('Must not include extra key "%s" to create.', key);
        }
    });
};
...
/**
 * Ignore all keys that are not editable
 * Check each editable key, if present, for valid value
 * Check to make sure that we don't have extra keys
 *
 * @param schema
 * @param resource
 ...
 * @throws error
 */
exports.filterUpdatedResourceThroughSchema = function (schema, resource) {
    if (_.keys(resource).length === 0) {
        throw new VError('Must include at least one key to update');
    }
    ...
    _.each(schema, function (parameters, key) {
        if (_.has(resource, key)) {
            if (parameters.editable) {
                if (!validateType(parameters.type, resource[key])) {
                    throw new VError('Value for key "%s" is not valid.', key);
                } else {
                    result[key] = resource[key];
                }
            } // else ignore non-editable values
        }
    });

    _.each(resource, function (value, key) {
        if (!_.has(schema, key)) {
            throw new VError('Must not include extra key "%s" on update.', key);
        }
    });
