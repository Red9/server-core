define(['vendor/jquery', 'vendor/underscore', 'sandbox', 'vendor/jquery.validate'], function($, _, sandbox) {
    function modifyResource(myPlace, configuration) {
        this.showModal(myPlace, configuration);
    }
    ...
    modifyResource.prototype.showModal = function(myPlace, configuration) {
        var parameters = {
        ...
        sandbox.requestTemplate('modals/modifyresource', function(template) {
            if (configuration.resourceType === 'event') {
                $.ajax({// This is a hack, and should be moved to the API
                    ...
                    success: function(eventTypes) {
                        ...
                        myPlace.find('form').validate(schemas.event(amCreating));
                    }
                });
