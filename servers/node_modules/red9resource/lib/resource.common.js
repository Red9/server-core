    ...
    _.each(query, function (parameter, key) {
        ...
        if (_.isObject(parameter)) {
            ...
            if (Object.keys(parameter).length === 1
                ...
                result += key + ' IN (' + _.reduce(parameter.$in, function (memo, value) {
                    ...
                    parameters.push(
                        {
                            value: convertJSTypeToCassandraType(value, map.jsType, map.cassandraType),
                            hint: map.cassandraType
                        });
...
exports.createResourceQuery = function (tableName, mapping, newResource) {
    var result = _.reduce(mapping, function (memo, map, index) {
            if (_.has(newResource, map.jsKey)) {
                ...
                memo.parameterPlaceholders += '?';
                memo.parameters.push({
                    value: convertJSTypeToCassandraType(newResource[map.jsKey], map.jsType, map.cassandraType),
                    hint: map.cassandraType
                });
            }
            return memo;
...
exports.createUpdateQuery = function (tableName, id, mapping, updatedResource) {
    var result = _.reduce(mapping, function (memo, map, index) {
            if (_.has(updatedResource, map.jsKey)) {
                ...
                memo.parameters.push({
                    value: convertJSTypeToCassandraType(updatedResource[map.jsKey], map.jsType, map.cassandraType),
                    hint: map.cassandraType
                });
            }
            return memo;
