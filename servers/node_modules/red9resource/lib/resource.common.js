...
function convertCassandraTypeToJSType(value, jsType, cassandraType) {
    if (_.isNull(value)) {
        if (cassandraType.indexOf('set') !== -1) {
            // At this point, we know that cassandraResource does not have a
            // matching key. From the Cassandra documentation, an empty array
            // and null are indistinguishable. So, we need to replace null
            // with an array.
            return [];
        } else {
            return; // undefined
