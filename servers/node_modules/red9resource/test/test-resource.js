...
function runEndToEnd(self, test, name, inputResource, modifyKey, modifyValue) {
    test.expect(11);
    self.sut.create(inputResource, function (err, createdResource) {
        if (err) { // A little un orthodox, but this has bitten me several times.
            console.log('Error in created resource. Bad Cassandra key? ' + err);
            console.dir(createdResource);
        }

        test.strictEqual(err, null);
        self.sut.find({id: createdResource.id}, null,
            function (searchResult) {
                // The created resource should have, at a minimum, the same keys
                // as the input resource
                test.equal(
                    _.pick(createdResource, _.keys(inputResource)).length,
                    inputResource.length
                );

                //test.deepEqual(removeDynamicFields(self.sut.schema, searchResult), createdResource, 'search result should match created ' + name);
            },
            function (err, rowCount) {
                ...
                self.sut.update(createdResource.id, updateValue, function (err) {
                    test.strictEqual(err, null);
                    self.sut.find({id: createdResource.id}, null,
                        function (event) {
                        ...
                        function (err, rowCount) {
                            ...
                            self.sut.delete(createdResource.id, function (err) {
                                test.strictEqual(err, null);
                                self.sut.find({id: createdResource.id}, null, null,
                                    function (err, rowCount) {
                                        test.strictEqual(err, null);
...
exports['resource CRUD tests'] = {
    ...
    'comment': function (test) {
        var inputComment = {
            author: '46ed9537-a782-431f-8746-445e8aa28d9c',
            resourceType: 'dataset',
            resource: 'f6a3db1a-0d8e-43ec-bc3d-65d27cffed76',
            body: 'Hello, World!'
        };
    ...
    'video': function (test) {
        var inputVideo = {
            dataset: '0aa4053d-72a5-4ddb-96c3-18f1e47279da',
            host: 'YouTube',
            hostId: '12DB3ER7',
            startTime: 1234
        };
    ...
    // Can't test this, since it starts looking for datasets and RNC files.
    //'event': function (test) {
    //    var inputEvent = {
    //        "datasetId": "ead038a6-f069-8515-4d2f-8dc6154fed2e",
    //        "endTime": 1409040125393,
    //        "startTime": 1409040110033,
    //        "type": "Wave",
    //    };
