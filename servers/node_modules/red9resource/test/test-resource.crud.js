...
exports['resource.crud find'] = {
    ...
    'correct CQL statement': function (test) {
        ...
        var correctCQL = "SELECT * FROM event WHERE type=? AND id=? ALLOW FILTERING";
...
/**
 * This test set has lost much of it's value. It used to be that I was using
 * this resource layer to do validity checks. But at this point I much prefer
 * to just let others take care of it. The DB will complain if we try to give
 * it data that doesn't fit.
 *
 * So, I changed the checks (!err), but really these tests aren't too useful.
 *
 */
exports['resource.crud create'] = {
    ...
    'missing keys sent to create do not matter': function (test) {
        function callback(err) {
            test.ok(!err, 'error must not be defined');
        }
    ...
    'too many keys or non-create keys created do not matter': function (test) {
        function callback(err) {
            test.ok(!err, 'error must not be defined');
        }
