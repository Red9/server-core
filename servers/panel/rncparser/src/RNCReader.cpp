...
void RNCReader::readHeader(rapidjson::Document &d, std::ifstream *rnc) {
    char json[10000];
    char *tail = json;


    rnc->read(tail, bufferEndSize);
    tail += bufferEndSize;

    while (rnc->good() && memcmp(tail - bufferEndSize, bufferEnd, bufferEndSize) != 0) {
        rnc->read(tail, 1);
        tail++;
        // TODO(SRLM): Add in a check to make sure that all the recieved characters are valid
    }
    ...
    //std::cout << "Buffer strlen = " << strlen(json) << "\n";
...
RNCState *RNCReader::parseHeader(rapidjson::Document &header) {
    ...
    struct tm timeDate;
    strptime(createTimeString.c_str(), "%FT%T%z", &timeDate);
    int64_t baseTime = mktime(&timeDate);
