define(['vendor/jquery', 'sandbox', 'vendor/jquery.validate'], function($, sandbox) {
    ...
    function downloadPanelModal(myPlace, configuration) {
        this.myPlace = myPlace;

        this.resourceDownload(configuration);
    }
    ...
    downloadPanelModal.prototype.resourceDownload = function(configuration) {
        var self = this;

        sandbox.requestTemplate('modals/downloadpanel', function(template) {
            ...
            self.myPlace.html(template(configuration.resource));
    ...
    downloadPanelModal.prototype.processForm = function() {
        ...
        this.myPlace.find('[data-name=download-perma-link]').attr('href', url).removeClass('hide');
